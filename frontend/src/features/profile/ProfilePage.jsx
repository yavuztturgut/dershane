import { Button, Group, Paper, PasswordInput, SimpleGrid, Text, TextInput, Title } from '@mantine/core';
...
export function ProfilePage() {
  ...
  return <div className="max-w-3xl"><Title order={1} className="text-2xl" mb="lg">{t('profile')}</Title><SimpleGrid cols={{ base: 1, md: 2 }}>
    <Paper withBorder p="lg"><Title order={2} className="text-lg">{t('accountDetails')}</Title><Text size="sm" c="dimmed" mt="xs">{t('role')}: {user.role_name} · {t('class')}: {user.class_name || '-'}</Text><form onSubmit={profileForm.onSubmit(saveProfile)}><TextInput label={t('name')} required mt="lg" {...profileForm.getInputProps('name')} /><TextInput label={t('email')} required mt="md" {...profileForm.getInputProps('email')} /><Group justify="flex-end" mt="lg"><Button type="submit" loading={isUpdatingProfile}>{t('save')}</Button></Group></form></Paper>
    <Paper withBorder p="lg"><Title order={2} className="text-lg">{t('changePassword')}</Title><form onSubmit={passwordForm.onSubmit(savePassword)}><PasswordInput label={t('currentPassword')} required mt="lg" {...passwordForm.getInputProps('current_password')} /><PasswordInput label={t('newPassword')} required mt="md" {...passwordForm.getInputProps('new_password')} /><PasswordInput label={t('confirmPassword')} required mt="md" {...passwordForm.getInputProps('confirm')} /><Group justify="flex-end" mt="lg"><Button type="submit" loading={isChangingPassword}>{t('changePassword')}</Button></Group></form></Paper>
  </SimpleGrid></div>;
}
